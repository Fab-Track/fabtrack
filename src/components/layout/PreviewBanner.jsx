...
export default function PreviewBanner() {
  ...
  const { isImpersonating, impersonatedEmployee, exitImpersonation } = useImpersonation();

  // Impersonation banner takes priority
  if (isImpersonating) {
    return (
      <div className="fixed top-14 md:top-0 left-0 right-0 md:left-56 z-[90] bg-amber-400 text-amber-950 flex items-center justify-between px-4 py-2 shadow-md">
        <div className="flex items-center gap-2 text-sm font-semibold">
          ...
          <span>Viewing as: {impersonatedEmployee.name} ({impersonatedEmployee.role?.replace(/_/g, " ") || "Employee"})</span>
          <span className="text-amber-700 font-normal text-xs ml-1">— Live data view</span>
        </div>
        <button
